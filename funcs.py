...
def parse_line(text: str) -> dict:
    ...
    command_module = command_name + pp.Optional(full_argument_name)('arg') + pp.Optional(pp.Suppress(';'))

    comment_module = comment_name('comm')

    full_parse_module = pp.Optional(label_name) + pp.Optional(command_module) + pp.Optional(comment_module)
...
def recognize_args(args: list[str]) -> list[dict]:
    ...
    variable_name = pp.Combine(pp.Char(pp.alphas) + pp.Optional(pp.Word(pp.alphas + pp.nums + "_")))
    # Сдвиги
    shift_name = pp.Combine(pp.Optional('-') + pp.Word(pp.nums))

    # Шаблоны для определения моды
    modes_list = [
        ("@" + shift_name + "(" + register_name + ")").setParseAction(pp.replaceWith('7')),
    ...
    full_reg_name = pp.Optional(pp.Suppress(pp.Word('@-+('))) + register_name + pp.Optional(pp.Suppress(pp.Word('+)')))
    ...
    const_var_name = pp.Suppress(pp.Optional(pp.Word('#@'))) + (pp.Combine(pp.Optional('-') + pp.Word(pp.nums))("const") | variable_name)
    
    symbol_name = "#\'" + pp.Char(pp.printables)("symbol")

    names_to_search = mode_7 | mode_6 | full_reg_name | symbol_name | const_var_name | symbol_name
